...
This command will create two files called `exons.gtf` and `introns.gtf` at the predefined `PATH`, which can be used as canonical GTF annotation for gene quantification using any preferred quantifier tool able to process GTF files as input annotation format (tested on FeatureCounts tool [[5]])
...
This function allows users to quantify the level of expression of their aligned reads by making use of custom-created exonic/intronic GTF annotation files. The featureCounts built-in function from Rsubread package [[4]] is used for such purpose. Users may optionally decide to use any other preferred quantification pipeline provided they use the exonic/intronic GTF annotation files previously generated as a reference. Then, users can input their count matrices in further steps.
