...
getEISAcomp <- function(exons, introns, design, capOut=FALSE,
                        filterExpr=TRUE, batch=NULL, percent=0.5, cpm=1){
  ...
  if(filterExpr==TRUE){
    ...
    if(is.null(batch) == FALSE){
      ...
      design.batch <- model.matrix(~., batch)
      design.batch <- design.batch[,-1]
